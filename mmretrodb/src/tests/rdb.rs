...
fn test_scan_by_crc() {
    ...
    assert_eq!(rom.name(), "Akira (Japan)[h][t][u][mapper 4]");
    assert_eq!(rom.region(), "Japan");
    assert_eq!(rom.rom_name(), "Akira (1988-12-24)(Taito)(JP)[h][t][u][mapper 4].nes");
    assert_eq!(rom.size(), 393232);
    assert_eq!(rom.crc(), 872268823);
    assert_eq!(rom.md5(), &[9, 39, 224, 85, 166, 204, 70, 214, 82, 71, 217, 112, 25, 231, 176, 93]);
    assert_eq!(rom.sha1(), &[164, 30, 8, 75, 97, 19, 36, 36, 48, 180, 118, 211, 176, 218, 28, 141, 184, 45, 168, 61]);
    assert_eq!(rom.release().date(), "1988-12");
}
